...
int WorldSocket::open(void *a)
{
    ...
    WorldPacket data(MSG_TRANSFER_INITIATE);
...
int WorldSocket::handle_input_payload(void)
{
    ...
    {
        STRAWBERRY_ASSERT(m_RecvPct.space() == 0);
        STRAWBERRY_ASSERT(m_WorldHeader.space() == 0);
        STRAWBERRY_ASSERT(m_RecvWPct != NULL);
    ...
    try
    {
        switch(opcode)
        {
            ...
            default:
            {
                ...
                {
                    sLog.outError("WorldSocket::ProcessIncoming: Client not authed opcode = %u", opcode);
                    return -1;
                }
            }
        }
