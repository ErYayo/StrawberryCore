...
void Battlefield::Update(uint32 uiDiff)
{
    ...
    else
    {
        ...
        if(m_preBattleTimer <= uiDiff && !m_invitationSent)
        {
            ...
            WorldPacket data(NUM_MSG_TYPES);//SMSG_BATTLEFIELD_MGR_ENTRY_INVITE);

            uint8 guidMask[] = { 5, 3, 7, 2, 6, 1, 0, 4 };
            uint8 guidBytes[] = { 1, 2, 5, 4, 7, 0, 3, 6 };

            data.WriteGuidMask(getGuid(),guidMask,8);
            data.FlushBits();

            data.WriteGuidBytes(getGuid(),guidBytes,2,0);
            data << uint32(time(NULL) + 20); // unk1
            data.WriteGuidBytes(getGuid(),guidBytes,6,2);
