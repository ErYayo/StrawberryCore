...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADTALENTS,         "SELECT talent_id, current_rank, spec, tabId FROM character_talent WHERE guid = '%u' ORDER BY tabId,spec", m_guid.GetCounter());
...
{
    WorldPacket data(NUM_MSG_TYPES);//SMSG_RESPONSE_CHARACTER_ENUM, 270);
...
void WorldSession::HandleResponseCharacterCreateOpcode( WorldPacket & recv_data )
{
    ...
    WorldPacket data(NUM_MSG_TYPES);//SMSG_RESPONSE_CHARACTER_CREATE, 1);                  // returned with diff.values in all cases
...
void WorldSession::HandleCharDeleteOpcode( WorldPacket & recv_data )
{
    ...
    //needed as the client no longer sends for char enum after del.
    CharacterDatabase.AsyncPQuery(&chrHandler, &CharacterHandler::HandleResponseCharacterEnumCallback, GetAccountId(),
        !sWorld.getConfig(CONFIG_BOOL_DECLINED_NAMES_USED) ?
        //   ------- Query Without Declined Names --------
        //           0               1                2                3                 4                  5                       6                        7
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    data.Initialize(NUM_MSG_TYPES);//SMSG_GET_LEARNED_DANCE_MOVES, 4+4);
    data << uint32(0);
    data << uint32(0);
    SendPacket(&data);
