...
void WorldSession::HandleLogoutCancelOpcode( WorldPacket & /*recv_data*/ )
{
    ...
    WorldPacket data(NUM_MSG_TYPES);//SMSG_LOGOUT_CANCEL, 0 );
    SendPacket( &data );
...
void WorldSession::HandleRealmSplitState( WorldPacket & recv_data )
{
    ...
    WorldPacket data(NUM_MSG_TYPES);//SMSG_REALM_SPLIT_MSG, 4+4+split_date.size()+1);
    data << unk;
...
void WorldSession::HandleUpdateObjectFailure(WorldPacket & recv_data)
{
    ObjectGuid guid;
    BitStream mask = recv_data.ReadBitStream(8);
    ByteBuffer bytes(8, true);
