...
void WorldSession::SendNameCacheOpcode(Player *p)
{
    if(!p)
        return;
                                                            // guess size
    WorldPacket data(NUM_MSG_TYPES);//SMSG_NAME_CACHE, (8+1+1+1+1+1+10) );
...
void WorldSession::SendNameCacheOpcodeFromDBCallBack(QueryResult *result, uint32 accountId)
{
    ...
    else
    {
        ...
        pClass       = fields[4].GetUInt8();
                                                            ...
                                                            // guess size
    WorldPacket data(NUM_MSG_TYPES);//SMSG_NAME_CACHE, (8+1+1+1+1+1+1+10) );
...
void WorldSession::HandleCreatureStatsOpcode( WorldPacket & recv_data )
{
    ...
    if (ci)
    {
        ...
        DETAIL_LOG("WORLD: CMSG_CREATURE_STATS '%s' - Entry: %u.", ci->Name, entry);
                                                            // guess size
        WorldPacket data(NUM_MSG_TYPES);//SMSG_CREATURE_STATS, 100 );
    ...
    else
    {
        ...
        WorldPacket data(NUM_MSG_TYPES);//SMSG_CREATURE_STATS, 4 );
...
void WorldSession::HandleGameObjectStatsOpcode( WorldPacket & recv_data )
{
    ...
    if(info)
    {
        ...
        WorldPacket data (NUM_MSG_TYPES);//SMSG_GAME_OBJECT_STATS, 150);
        data << uint32(entryID);
    ...
    else
    {
        ...
        WorldPacket data (NUM_MSG_TYPES);//SMSG_GAME_OBJECT_STATS, 4 );
...
void WorldSession::HandlePageTextQueryOpcode( WorldPacket & recv_data )
{
    ...
    while (pageID)
    {
        PageText const *pPage = sPageTextStore.LookupEntry<PageText>( pageID );
                                                            // guess size
        WorldPacket data(NUM_MSG_TYPES);//SMSG_PAGE_TEXT_CACHE, 50);
        data << pageID;
