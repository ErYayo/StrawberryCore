...
void Spell::AddUnitTarget(Unit* pVictim, SpellEffectIndex effIndex)
{
    ...
    if (!spellEffect || spellEffect->Effect == 0 || !spellMisc)
        return;
    ...
    if (spellMisc->speed > 0.0f && affectiveObject && pVictim != affectiveObject)
    {
    ...
    switch(targetMode)
    {
        ...
        case TARGET_AREAEFFECT_GO_AROUND_DEST:
        {
            ...
            for (SpellScriptTarget::const_iterator i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
            {
                if (i_spellST->second.type == SPELL_TARGET_TYPE_GAMEOBJECT)
                {
                    ...
                    Strawberry::GameObjectEntryInPosRangeCheck go_check(*m_caster, i_spellST->second.targetEntry, x, y, z, radius);
        ...
        case TARGET_TABLE_X_Y_Z_COORDINATES:
        {
            if (SpellTargetPosition const* st = sSpellMgr.GetSpellTargetPosition(m_spellInfo->Id))
            {
                ...
                if (st->target_mapId != m_caster->GetMapId() && spellEffect->Effect != SPELL_EFFECT_TELEPORT_UNITS)
        ...
        case TARGET_EFFECT_SELECT:
        {
            ...
            switch(spellEffect->Effect)
            {
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_PARTY:
                    // AreaAura
                    if ((spellMisc && spellMisc->Attributes == (SPELL_ATTR_NOT_SHAPESHIFT | SPELL_ATTR_UNK18 | SPELL_ATTR_CASTABLE_WHILE_MOUNTED | SPELL_ATTR_CASTABLE_WHILE_SITTING)) || (spellMisc && spellMisc->Attributes == SPELL_ATTR_NOT_SHAPESHIFT))
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if(Unit *target = m_targets.getUnitTarget())
    {
        ...
        if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_UNK20) && m_spellInfo->HasAttribute(SPELL_ATTR_EX_UNK9) && target->HasInArc(M_PI_F, m_caster))
        {
                ...
                for(SpellScriptTarget::const_iterator i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
                {
                    switch(i_spellST->second.type)
                    {
                        ...
                        default:
                        {
                            ...
                            if (!targetExplicit)
                            {
                                Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster, i_spellST->second.targetEntry, i_spellST->second.type != SPELL_TARGET_TYPE_DEAD, i_spellST->second.type == SPELL_TARGET_TYPE_DEAD, range);
...
SpellCastResult Spell::CheckPower()
{
    ...
    if (m_spellInfo->GetPowerType() == POWER_RUNES)
    {
...
bool Spell::IsNeedSendToClient() const
{
    ...
    return m_spellInfo->GetSpellVisual(0) || m_spellInfo->GetSpellVisual(1) || IsChanneledSpell(m_spellInfo) ||
        spellMisc && spellMisc->speed > 0.0f || (!m_triggeredByAuraSpell && !m_IsTriggeredSpell);
}
