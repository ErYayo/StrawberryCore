...
void ScriptedAI::DoPlaySoundToSet(WorldObject* pSource, uint32 uiSoundId)
{
    ...
    {
        error_log("SSC: Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", uiSoundId, pSource->GetTypeId(), pSource->GetGUIDLow());
        return;
    }
...
SpellEntry const* ScriptedAI::SelectSpell(Unit* pTarget, int32 uiSchool, int32 iMechanic, SelectTarget selectTargets, uint32 uiPowerCostMin, uint32 uiPowerCostMax, float fRangeMin, float fRangeMax, SelectEffect selectEffects)
{
    ...
    for (uint8 i = 0; i < 4; ++i)
    {
        ...
        if (uiSchool >= 0 && pTempSpell->GetSpellSchoolMask() & uiSchool)
            continue;
...
void FillSpellSummary()
{
    ...
    for (uint32 i = 0; i < GetSpellStore()->GetNumRows(); ++i)
    {
        ...
        for (uint8 j = 0; j < 3; ++j)
        {
            SpellEffectEntry const* spellEffect = pTempSpell->GetSpellEffect(SpellEffectIndex(j));
            if (!spellEffect)
                continue;
            ...
            if (spellEffect->EffectImplicitTargetA == TARGET_CHAIN_DAMAGE ||
                spellEffect->EffectImplicitTargetA == TARGET_CURRENT_ENEMY_COORDINATES ||
                spellEffect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA ||
                spellEffect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA_INSTANT ||
                spellEffect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES ||
                spellEffect->EffectImplicitTargetA == TARGET_ALL_ENEMY_IN_AREA_CHANNELED)
            ...
            if (spellEffect->EffectImplicitTargetA == TARGET_ALL_PARTY_AROUND_CASTER ||
            ...
            if (spellEffect->EffectImplicitTargetA == TARGET_SELF ||
                spellEffect->EffectImplicitTargetA == TARGET_SINGLE_FRIEND ||
                spellEffect->EffectImplicitTargetA == TARGET_SINGLE_PARTY ||
                spellEffect->EffectImplicitTargetA == TARGET_ALL_PARTY_AROUND_CASTER ||
                spellEffect->EffectImplicitTargetA == TARGET_AREAEFFECT_PARTY ||
                spellEffect->EffectImplicitTargetA == TARGET_CASTER_COORDINATES)
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        error_log("SSC: DoResetThreat called for creature that either cannot have threat list or has empty threat list (m_creature entry = %d)", m_creature->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float fX, float fY, float fZ, float fO)
{
    ...
    {
        error_log("SSC: %s tried to teleport non-player (%s) to x: %f y:%f z: %f o: %f. Aborted.", m_creature->GetGuidStr().c_str(), pUnit->GetGuidStr().c_str(), fX, fY, fZ, fO);
        return;
    }
...
Unit* ScriptedAI::DoSelectLowestHpFriendly(float fRange, uint32 uiMinHPDiff)
{
    ...
    Strawberry::MostHPMissingInRangeCheck u_check(m_creature, fRange, uiMinHPDiff);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    Strawberry::FriendlyCCedInRangeCheck u_check(m_creature, fRange);
    Strawberry::CreatureListSearcher<Strawberry::FriendlyCCedInRangeCheck> searcher(pList, u_check);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyMissingBuff(float fRange, uint32 uiSpellId)
{
    ...
    Strawberry::FriendlyMissingBuffInRangeCheck u_check(m_creature, fRange, uiSpellId);
    Strawberry::CreatureListSearcher<Strawberry::FriendlyMissingBuffInRangeCheck> searcher(pList, u_check);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    Strawberry::AnyPlayerInObjectRangeCheck check(m_creature, fMinimumRange);
    Strawberry::PlayerSearcher<Strawberry::AnyPlayerInObjectRangeCheck> searcher(pPlayer, check);
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(const uint32 uiDiff)
{
    ...
    switch(m_creature->GetEntry())
    {
        ...
        case NPC_ANUBARAK:
            ...
            break;
        default:
            error_log("SSC: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition.", m_creature->GetEntry());
            return false;
    }
