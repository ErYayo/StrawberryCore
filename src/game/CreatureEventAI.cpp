    ...
    for (CreatureEventAIList::iterator i = m_CreatureEventAIList.begin(); i != m_CreatureEventAIList.end(); ++i)
        if ((*i).Event.event_type == EVENT_T_SPELLHIT)
            ...
            if (!(*i).Event.spell_hit.spellId || pSpell->Id == (*i).Event.spell_hit.spellId)
                if (pSpell->GetSpellSchoolMask() & (*i).Event.spell_hit.schoolMask)
                    ProcessEvent(*i, pUnit);
}
...
bool CreatureEventAI::CanCast(Unit* Target, SpellEntry const *Spell, bool Triggered)
{
    ...
    //Check for power
    if (!Triggered && m_creature->GetPower((Powers)Spell->GetPowerType()) < Spell::CalculatePowerCost(Spell, m_creature))
        return false;

    SpellRangeEntry const *TempRange = NULL;

    TempRange = GetSpellRangeStore()->LookupEntry(Spell->GetRangeIndex());
